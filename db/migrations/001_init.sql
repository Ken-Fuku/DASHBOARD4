...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'company', json_object('id', NEW.id), 'I', json_object('id', NEW.id, 'company_code', NEW.company_code, 'name', NEW.name), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'company', json_object('id', NEW.id), 'U', json_object('id', NEW.id, 'company_code', NEW.company_code, 'name', NEW.name), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'company', json_object('id', OLD.id), 'D', json_object('id', OLD.id, 'company_code', OLD.company_code, 'name', OLD.name), 1, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'store', json_object('id', NEW.id), 'I', json_object('id', NEW.id, 'store_code', NEW.store_code, 'company_id', NEW.company_id, 'name', NEW.name), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'store', json_object('id', NEW.id), 'U', json_object('id', NEW.id, 'store_code', NEW.store_code, 'company_id', NEW.company_id, 'name', NEW.name), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'store', json_object('id', OLD.id), 'D', json_object('id', OLD.id, 'store_code', OLD.store_code, 'company_id', OLD.company_id, 'name', OLD.name), 1, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'visit_daily', json_object('id', NEW.id), 'I', json_object('id', NEW.id, 'store_id', NEW.store_id, 'visit_date', NEW.visit_date, 'visitors', NEW.visitors), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'visit_daily', json_object('id', NEW.id), 'U', json_object('id', NEW.id, 'store_id', NEW.store_id, 'visit_date', NEW.visit_date, 'visitors', NEW.visitors), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'visit_daily', json_object('id', OLD.id), 'D', json_object('id', OLD.id, 'store_id', OLD.store_id, 'visit_date', OLD.visit_date, 'visitors', OLD.visitors), 1, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'budget_monthly', json_object('id', NEW.id), 'I', json_object('id', NEW.id, 'store_id', NEW.store_id, 'year_month', NEW.year_month, 'budget_amount', NEW.budget_amount), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'budget_monthly', json_object('id', NEW.id), 'U', json_object('id', NEW.id, 'store_id', NEW.store_id, 'year_month', NEW.year_month, 'budget_amount', NEW.budget_amount), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'budget_monthly', json_object('id', OLD.id), 'D', json_object('id', OLD.id, 'store_id', OLD.store_id, 'year_month', OLD.year_month, 'budget_amount', OLD.budget_amount), 1, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'budget_factors', json_object('id', NEW.id), 'I', json_object('id', NEW.id, 'store_id', NEW.store_id, 'year_month', NEW.year_month, 'factor_json', NEW.factor_json), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'budget_factors', json_object('id', NEW.id), 'U', json_object('id', NEW.id, 'store_id', NEW.store_id, 'year_month', NEW.year_month, 'factor_json', NEW.factor_json), 0, datetime('now'));
END;
...
BEGIN
  INSERT INTO change_log (tx_id, table_name, pk_json, op, payload, tombstone, created_at)
  VALUES (lower(hex(randomblob(16))), 'budget_factors', json_object('id', OLD.id), 'D', json_object('id', OLD.id, 'store_id', OLD.store_id, 'year_month', OLD.year_month, 'factor_json', OLD.factor_json), 1, datetime('now'));
END;
